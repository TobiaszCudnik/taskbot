...
interface Loki extends LokiEventEmitter {
  ...
  anonym<T>(
    docs: T | T[],
    indexesArray?: LokiCollectionOptions
  ): LokiCollection<T>
...
interface LokiPersistenceInterface {
  loadDatabase(
    dbname: string,
    callback: (dataOrErr: string | Error) => void
  ): void
  saveDatabase(
    dbname: string,
    dbstring: string,
    callback: (resOrErr: void | Error) => void
  ): void
  deleteDatabase(
    dbname: string,
    callback?: (resOrErr: void | Error) => void
...
interface LokiResultset<E> {
   ...
   */
  new <E>(collection: LokiCollection<E>, options: LokiResultsetOptions<E>):
    | LokiResultset<E>
    | E[]
  ...
  map<T>(
    mapFun: (currentValue: E, index: number, array: E[]) => T
  ): LokiResultset<T>
}
...
interface LokiCollection<E> extends LokiEventEmitter {
  ...
  asyncListeners: boolean
  binaryIndices: {
    [id: string]: { name: string; dirty: boolean; values: number[] }
  }
   ...
   */
  findAndUpdate(
    filterFunction: (obj: E) => boolean,
    updateFunction: (obj: E) => void
  ): void
...
interface LokiIndexedAdapter {
   ...
   */
  getCatalogSummary(
    callback: (entries: { app: string; key: string; size: number }) => void
  ): void
}
...
interface LokiCatalog {
  ...
  getAppKeyById<T>(
    id: any,
    callback: (result: any, data: T) => void,
    data: T
  ): void
...
interface LokiCryptedFileAdapter {
  ...
  loadDatabase(
    dbname: string,
    callback: (decryptedDataOrErr: string | any) => void
  ): void
...
interface LokiJquerySyncAdapter {
  ...
  saveDatabase(
    name: string,
    data: any,
    callback?: (data: any, textStatus: string, xhr: XMLHttpRequest) => any
  ): void
